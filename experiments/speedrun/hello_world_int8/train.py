...
Trains multiple Llama model sizes with INT8 quantization and default optimizer (Chinchilla optimal).
"""
...
for name, model_cfg, param_count in MODEL_SWEEP:
    ...
    SPEEDRUN_CONFIGS[name] = SpeedrunConfig(
        author=AUTHOR,
        description=f"Chinchilla optimal run with Int8 ({name})",
...
if __name__ == "__main__":
    ...
    for name, speedrun_config in SPEEDRUN_CONFIGS.items():
        submission_name = f"hello_world_int8_{name}"
