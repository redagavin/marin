...
"""
Speedrun code for batch size ablation study using a 50M parameter model.
Trains on Fineweb-Edu dataset with 2x Chinchilla optimal tokens (2B tokens).
Accepts batch_size as a command-line argument and adjusts learning rate accordingly.
"""
...
def calculate_hyperparameters(batch_size: int, param_count: int = 50_000_000, seq_len: int = 1024, warmup_fraction: float = 0.05):
    """
    Calculate hyperparameters based on batch size.
    ...
    # Scale learning rate by sqrt(batch_size / base_batch_size)
    # Base: batch_size=128, learning_rate=0.020
    base_batch_size = 128
    base_learning_rate = 0.020
    learning_rate = base_learning_rate * math.sqrt(batch_size / base_batch_size)
...
def get_batch_size_from_args() -> int:
    ...
    if len(sys.argv) < 2 or not sys.argv[1].lstrip('-').isdigit():
        print("Usage: python trial_batch_size_muon.py <batch_size> [--prefix ...]")
        print("Example: python trial_batch_size_muon.py 256 --prefix /output")
...
hp = calculate_hyperparameters(BATCH_SIZE, PARAM_COUNT, SEQ_LEN)
NUM_TRAIN_STEPS = hp["num_steps"]
LEARNING_RATE = hp["learning_rate"]
WARMUP_STEPS = hp["warmup_steps"]

print(f"\n{'='*60}")
print(f"Batch Size Ablation Study - 50M Model")
print(f"{'='*60}")
print(f"Batch Size:       {BATCH_SIZE}")
print(f"Learning Rate:    {LEARNING_RATE:.6f} (base: 0.020000)")
...
# Muon optimizer config scaled for 50M model
...
speedrun_config = SpeedrunConfig(
    ...
    description=f"50M model batch size {BATCH_SIZE} (2x Chinchilla optimal tokens)",
...
if __name__ == "__main__":
    ...
    experiment_name = f"trial_batch_size_muon_sweep_warmup_bs{BATCH_SIZE}"
